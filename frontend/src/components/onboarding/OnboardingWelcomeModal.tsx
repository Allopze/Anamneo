...
export default function OnboardingWelcomeModal({ onClose }: OnboardingWelcomeModalProps) {
  ...
  return (
    ...
    >
      <div className="relative w-full max-w-md overflow-hidden rounded-card border border-surface-muted/40 bg-surface-elevated shadow-elevated">
        <button
          type="button"
          onClick={onClose}
        ...
        <div className="px-6 pb-6 pt-8">
          ...
          <h2 id="welcome-modal-title" className="text-xl font-bold text-ink">
            Bienvenido/a a Anamneo
          </h2>
          <p className="mt-2 text-sm leading-5 text-ink-secondary">
            Preparamos una guía de {progress.steps.length} pasos para ayudarte a comenzar.
          ...
          <div className="mt-5 space-y-2">
            ...
            {progress.steps.length > 3 && (
              <p className="px-1 text-xs text-ink-muted">
                y {progress.steps.length - 3} pasos más…
              </p>
            )}
          </div>
