...
exports.EncountersPdfService = void 0;
...
let EncountersPdfService = class EncountersPdfService {
    constructor(prisma) {
        this.prisma = prisma;
    }
    async generatePdf(encounterId, user) {
        ...
        return new Promise((resolve, reject) => {
            ...
            doc
                .fontSize(10)
                .font('Helvetica')
                .text(`Fecha: ${new Date(encounter.createdAt).toLocaleDateString('es-CL', { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' })}`, { align: 'center' });
            doc.moveDown(0.5);
            doc
                .moveTo(doc.x, doc.y)
            ...
            if (revEntries.length > 0) {
                for (const [key, value] of revEntries) {
                    const text = typeof value === 'object' && value !== null
                        ? value.texto || value.observaciones
                        : value;
                    if (text) {
                        const label = key
                            .replace(/([A-Z])/g, ' $1')
            ...
            if (!trat.plan && !trat.indicaciones && !trat.receta)
                doc.text('-');
            doc.moveDown(0.5);
...
exports.EncountersPdfService = EncountersPdfService;
exports.EncountersPdfService = EncountersPdfService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [prisma_service_1.PrismaService])
], EncountersPdfService);
//# sourceMappingURL=encounters-pdf.service.js.map