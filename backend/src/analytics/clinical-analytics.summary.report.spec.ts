...
describe('clinical-analytics.summary.report', () => {
  it('exports a shareable markdown report and audits the download', async () => {
    const auditService = {
    ...
    const markdown = await exportClinicalAnalyticsSummaryMarkdownReadModel({
      prisma: {} as never,
    ...
    expect(markdown).toContain('# Reporte de analítica clínica');
    expect(markdown).toContain('Hipertensión arterial');
    expect(markdown).toContain('Los resultados son descriptivos y observacionales');
  });
});