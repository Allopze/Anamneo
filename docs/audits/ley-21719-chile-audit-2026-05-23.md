...
- `npm --prefix backend test -- --runInBand clinical-analytics.read-model.spec.ts clinical-analytics.cases.read-model.spec.ts patients-list-read-model.spec.ts encounters-section-mutations.spec.ts` ✅
- `npm --prefix frontend test -- --runInBand pacientes-list.test.tsx` ✅
...
3. **Producto no bloqueante:** reasignacion interna de medico, checklist pre-consulta, permisos finos por accion, saneamiento admin seguro, exportacion operativa por rango/medico y configuracion flexible de secciones.
