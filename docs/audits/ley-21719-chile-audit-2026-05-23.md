...
## Actualizacion 2026-05-26 — reconciliacion post-Phase C y opt-out analitico

Esta actualizacion corrige el snapshot final de `septies`: en el arbol actual las migraciones Phase C-F ya fueron movidas a `backend/prisma/migrations/` y `schema.prisma` ya no conserva las columnas plaintext transitorias de `Patient`, representante legal, firmante de consentimiento ni solicitante DSAR.

### Cierres tecnicos aplicados

| Item | Estado | Evidencia |
|---|---|---|
| Phase C-F drop plaintext | ✅ Aplicadas como migraciones activas | `backend/prisma/migrations/20260524000000_*` a `20260524030000_*`, `backend/prisma/migrations-pending/README.md` sin migraciones activas |
| Opt-out de analitica interna | ✅ `Patient.processingObjections.ANALITICA_INTERNA=true` excluye resumen y drill-down | `backend/src/analytics/clinical-analytics.read-model.ts`, `backend/src/analytics/clinical-analytics.cases.read-model.ts` |
| Busqueda administrativa por RUT exento | ✅ UI + query backend | `frontend/src/app/(dashboard)/pacientes/PatientsFilterPanel.tsx`, `backend/src/patients/patients-list-read-model.ts` |
| Comparacion de versiones de secciones | ✅ diff redacted por campos en historial de cambios | `backend/src/encounters/encounters-sanitize.ts`, `frontend/src/components/EncounterAuditTimeline.tsx` |
| Checklist pre-consulta | ✅ Seleccion previa + confirmacion antes de crear atencion preparada | `frontend/src/app/(dashboard)/atenciones/nueva/page.tsx` |
| Exportacion operativa admin | ✅ CSV de atenciones por rango/medico con auditoria | `backend/src/patients/patients-operational-export-read-model.ts`, `frontend/src/app/(dashboard)/pacientes/PatientsFilterPanel.tsx` |

### Verificacion

- `npm --prefix backend run typecheck` ✅
- `npm --prefix frontend run typecheck` ✅
- `npm --prefix backend test -- --runInBand clinical-analytics.read-model.spec.ts clinical-analytics.cases.read-model.spec.ts patients-list-read-model.spec.ts patients-operational-export-read-model.spec.ts encounters-section-mutations.spec.ts` ✅
- `npm --prefix frontend test -- --runInBand pacientes-list.test.tsx nueva-atencion.test.tsx` ✅

### Pendiente tras 2026-05-26

No queda pendiente tecnico bloqueante dentro del Gate Go/No-Go. Lo restante se mantiene en tres grupos:

1. **Legal postergado:** texto final politica v1.0/anexos, RAT, DPIA, DPO, DPAs, matriz de finalidades opcionales.
2. **Operativo:** activar `REGULATORY_CONSENT_ENFORCEMENT=hard`, capacitacion, paquete de fiscalizacion, restore drill y drills DSAR/brecha en staging.
3. **Producto no bloqueante:** reasignacion interna de medico, permisos finos por accion, saneamiento admin seguro y configuracion flexible de secciones.
